        ...
        for (top, right, bottom, left), name in zip(self.face_locations, self.face_names):
            ...
            if name == "Unknown":
                try:
                    ...
                    # mosaic_img = frame[top:bottom, left:right]         # 이미지 자르고
                    # print("첫번째 "+str((right-left) // 30))
                    # print("두번째 "+str((top-bottom) // 30))
                    # height = mosaic_img.shape[0]
                    # width = mosaic_img.shape[1]
                    # mosaic_img = cv2.resize(mosaic_img, None ,fx = 5, fy = 5, interpolation=cv2.INTER_CUBIC)       # 지정 배율로 확대
                    # mosaic_img = mosaic_img[height, width]
                    # # mosaic_img = cv2.resize(frame, None, fx=0.2, fy=0.2, interpolation=cv2.INTER_AREA)     # 원래 크기로 resize
                    # frame[top:bottom, left:right] = mosaic_img          # 원래 이미지에 붙이기


                except Exception as e:
                    dimensions = mosaic_img.shape
                    height =mosaic_img.shape[0]
                    width = mosaic_img.shape[1]
                    channels = mosaic_img.shape[2]

                    print("height "+str(height))
                    print("width " + str(width))

                    # sample = cv2.pyrDown("./mosaicImage/sample_mozaic.jpg")
                    sample = cv2.imread("./mosaicImage/sample_mozaic.jpg")
                    sample = cv2.resize(sample, dsize=(height,width))
                    # sample = sample[height, width]

                    print("height " + str(height))
                    print("width "+ str(width))

                    frame[top:bottom, left:right] = sample
                    print(str(e))
            ...
            cv2.putText(frame, name, (left + 6, bottom - 6), font, 3, (0, 255, 255), 10)
